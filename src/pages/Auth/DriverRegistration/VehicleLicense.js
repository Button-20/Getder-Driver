...
const VehicleLicence = ({ step, setStep }) => {
  ...
  const uploadImage = async () => {
    ...
    if (!pickerImage.canceled) {
      setRegistration({
        ...registration,
        vehicle_license: `data:image/jpg;base64,${pickerImage.assets[0].base64}`,
      });
    }
  };
    ...
    <View style={styles.container}>
      <View>
        ...
        <View style={styles.imageArea}>
          {registration?.vehicle_license ? (
            <Image src={registration?.vehicle_license} style={styles.image} />
          ) : (
            <TouchableOpacity
        ...
        <Text style={styles.subHeading}>
          Please take a clear photo and avoid using flash
        </Text>
      </View>
      {registration?.vehicle_license && (
